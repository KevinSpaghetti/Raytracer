...
//Represents a mesh made by triangles
class Mesh {
public:
    Mesh() : vertices(),
             triangles(),
    ...
    Mesh(std::vector<Vertex>& vertices, std::vector<Triangle>& triangles, std::vector<Normal>& normals, std::vector<UV>& uvs) :
        vertices(vertices),
        triangles(triangles),
        normals(normals),
        uvs(uvs){}

    std::list<Intersection> intersect(const Ray& r){
